    ...
    if i < 41:
        if i < 10:
            ...
            locals()['a' + str(i)] = np.loadtxt(r'C:\Users\29860\Documents\1806\data_aligned\C5' + str(i - 16) + '.txt',
                                                skiprows=0)
# X_train = np.empty((42500,75),dtype="float32")
# y_train = np.empty((42500,1),dtype="float32")
...
X_train = a40[:170000:4, 1, np.newaxis]
y_train = a40[:170000:4, 3, np.newaxis]
data = np.concatenate((X_train, y_train), axis=1)
print('data', data.shape)

#获取训练集
def get_train_data(batch_size=60,time_step=75,train_begin=0,train_end=40000):
    batch_index=[]
...
#获取测试集
def get_test_data(time_step=75,test_begin=0):
...
#——————————————————训练模型——————————————————
def train_lstm(batch_size=80,time_step=75,train_begin=0,train_end=40000):
    ...
    with tf.Session() as sess:
        ...
        for i in range(500):
            ...
            print(i,loss_)
            if i % 50==0:
                print("保存模型：",saver.save(sess,'ckpt/newtry1.ckpt',global_step=i))

# train_lstm()

#————————————————预测模型————————————————————
def prediction(time_step=75):
    X=tf.placeholder(tf.float32, shape=[None,time_step,input_size])
    #Y=tf.placeholder(tf.float32, shape=[None,time_step,output_size])
    ...
    with tf.Session() as sess:
        ...
        # saver.restore(sess, module_file)
        new_saver = tf.train.import_meta_graph('ckpt/newtry1.ckpt-450.meta')
        new_saver.restore(sess, 'ckpt/newtry1.ckpt-450')
        test_predict=[]

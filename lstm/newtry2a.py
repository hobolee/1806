...
for ii in range(2, 2):
    ...
    y_train = a1[:170000:8, ii, np.newaxis]
    data = np.concatenate((X_train, y_train), axis=1)
    for i in range(2, 41):
        data = np.concatenate((data, np.concatenate((locals()['a'+str(i)][:170000:8, 1, np.newaxis], locals()['a'+str(i)][:170000:8, ii, np.newaxis]), axis=1)), axis=0)
    print('data', data.shape)

    #获取训练集
    def get_train_data(batch_size=64,time_step=150,train_begin=0,train_end=40000):
        batch_index=[]
    ...
    #——————————————————训练模型——————————————————
    def train_lstm(batch_size=80,time_step=150,train_begin=0,train_end=40000):
        ...
        with tf.Session() as sess:
            ...
            for i in range(10000):
                ...
                print(i,loss_)
                if i % 500==0:
                    print("保存模型：",saver.save(sess,'ckpt/newtry2_'+str(i)+'.ckpt',global_step=i))

    train_lstm()

    #————————————————预测模型————————————————————
    def prediction(time_step=150):
        X=tf.placeholder(tf.float32, shape=[None,time_step,input_size])
        #Y=tf.placeholder(tf.float32, shape=[None,time_step,output_size])

...
y_train = a1[:170000:8, 3, np.newaxis]
data = np.concatenate((X_train, y_train), axis=1)
for i in range(2, 17):
    data = np.concatenate((data, np.concatenate((locals()['a'+str(i)][:170000:8, 1, np.newaxis], locals()['a'+str(i)][:170000:8, 3, np.newaxis]), axis=1)), axis=0)
...
#——————————————————训练模型——————————————————
def train_lstm(batch_size=64,time_step=75,train_begin=0,train_end=21250*15):
    ...
    with tf.Session() as sess:
        ...
        for i in range(501):
            ...
            print(i,loss_)
            if i % 25==0:
                print("保存模型：",saver.save(sess,'ckpt/newtry3_sway2.ckpt',global_step=i))

# train_lstm()

#————————————————预测模型————————————————————
def prediction(time_step=75, test_begin=21250*0):
    X=tf.placeholder(tf.float32, shape=[None,time_step,input_size])
    #Y=tf.placeholder(tf.float32, shape=[None,time_step,output_size])

...
for i in range(1, 41):
    for j in range(42500):
        if i==40:
            X_test[j,:] = a40[0+j:300+j:4,1]
            y_test[j] = b40[300+j,2] 
        else:
            if i > 16:
                X_train[42500*(i-1)+j,:] = locals()['a'+str(i+1)][0+j:300+j:4,1]
            else:
                X_train[42500*(i-1)+j,:] = locals()['a'+str(i+1)][0+j:150+j:2,1]
            y_train[42500*(i-1)+j,:] = locals()['b'+str(i+1)][300+j,2]
#for i in range(1, 40):
#    del locals()['a'+str(i)]
#    del locals()['b'+str(i)] 
...
#[mo, pred] = run_network(model='test37a.h5', epochs =0)
